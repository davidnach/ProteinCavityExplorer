...
bool Verbose                  = FALSE;    // do we write processing notes to stdout?
...
void outputRecordsToFile(const list<PDBrec*>& l) {
  ...
  string pdbFilenameWH = pdbFileName.substr(0,pdbFileName.length()-4) + "H.pdb";
  char* pdbFilenameOutput = new char[pdbFilenameWH.length()+1];
  ofstream ofile;

  // create a char instance of the new file name
  pdbFilenameWH.copy(pdbFilenameOutput, pdbFilenameWH.length(), 0);

  // Prepare file for writing -- add headers
  ofile.open (pdbFilenameOutput);
  ofile << "REMARK   File Generated with REDUCE, downloaded from"<<endl;
  ...
  ofile << "REMARK   January 2008"<<endl;
  ofile << "REMARK   "<<endl;
  ofile << "REMARK   Original File name: " << pdbFileName<<endl;
  ofile << "REMARK   Please send comments to filip@cs.umass.edu"<<endl;
  ofile << "REMARK   "<<endl;
